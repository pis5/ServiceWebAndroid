...
@NamedQueries({
    ...
    @NamedQuery(name = "Participation.findEventByParticipant", query = "SELECT Distinct(p.evenement1)FROM Participation p WHERE p.personne = :participant"),
    ...
    @NamedQuery(name = "Participation.findOldEventByParticipant", query = "SELECT DISTINCT(p.evenement1) FROM Participation p WHERE (p.personne = :participant) AND p.evenement1.id < :offset order by p.evenement1.id DESC"),
    @NamedQuery(name = "Participation.findNewEventByParticipant", query = "SELECT DISTINCT(p.evenement1) FROM Participation p WHERE (p.personne = :participant) AND p.evenement1.id > :offset order by p.evenement1.id DESC")
})
